...
def np_pmpdb_exists(np_dir= np_dir):
    """Function to check if np files of the PMPDB dataset already exist.
...
def read_segmented_np():
    ''' Function to read the segmented PMHDB in form of numpy files created by script "create_np_files".
    Parameters
    ----------
...
def set_index(df):
    ''' Function to correctly set the index of the PMPDB data frames.
    Parameters
    ----------
